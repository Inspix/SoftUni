        ...
        static void ListToExcel(List<Student> students)
        {
            ...
            using (ExcelPackage package = new ExcelPackage(file))
            {
                ...
                using (ExcelRange xr = ws.Cells["A3:M3"])
                {
                    ...
                    xr.Style.Fill.BackgroundColor.SetColor(Color.GreenYellow);
                    xr.Style.Border.BorderAround(ExcelBorderStyle.Thin,Color.Black);
                    xr.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                }
                using (ExcelRange xr = ws.Cells[4,1,students.Count+3,6])
                {
                    xr.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                }
                ws.Column(1).Width = 5;
                ...
                for (int i = 0; i < students.Count; i++)
                {
                    for (int j = 1; j <= 13; j++)
                    {
                        ws.Cells[4 + i, j].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                        ws.Cells[4 + i, j].Style.Border.Bottom.Color.SetColor(Color.Black);
                    }
                    ws.Cells[4 + i, 1].Value = students[i].ID;
                    ws.Cells[4 + i, 1].Style.Border.Right.Style = ExcelBorderStyle.Medium;
                    ws.Cells[4 + i, 1].Style.Border.Right.Color.SetColor(Color.Black);
                ...
                using (ExcelRange xr = ws.Cells[4,13,students.Count+3,13])
                {
                    xr.Style.Fill.PatternType = ExcelFillStyle.Solid;
                    xr.Style.Fill.BackgroundColor.SetColor(Color.DarkGreen);
                    xr.Style.Border.BorderAround(ExcelBorderStyle.Thin,Color.Black);
                    xr.Style.Font.Color.SetColor(Color.AliceBlue);
                }
                package.Save();
            }
