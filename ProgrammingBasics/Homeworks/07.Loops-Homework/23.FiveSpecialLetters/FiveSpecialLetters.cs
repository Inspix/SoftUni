...
internal class FiveSpecialLetters
{
    ...
    private static int calcWeight(string x)
    {
        ...
        foreach (char item in x)
        {
            switch (item)
            {
                case 'a': weight += multiplier * 5; multiplier++;
                    break;

                case 'b': weight += multiplier * -12; multiplier++;
                    break;

                case 'c': weight += multiplier * 47; multiplier++;
                    break;

                case 'd': weight += multiplier * 7; multiplier++;
                    break;

                case 'e': weight += multiplier * -32; multiplier++;
                    break;
